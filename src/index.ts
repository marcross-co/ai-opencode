...
> {
  ...
  async *chatStream(
    ...
    try {
      ...
      yield {
        ...
        timestamp: Date.now(),
        error: {
          message: error.message || 'Unknown error during chat stream',
  ...
  async structuredOutput(
    ...
    const responseFormat: OpenAI.Chat.ChatCompletionCreateParams['response_format'] = options.outputSchema
      ? { 
          type: 'json_schema', 
          json_schema: { 
            name: 'response', 
